...
access(all) contract FlowtyRaffles {
    ...
    */
    ...
    access(all) resource interface RaffleSourcePrivate {
        ...
        to your raffle
        */
        access(RevealRaffle) fun revealCallback(drawingResult: DrawingResult)
        ...
        */
        access(AddEntry) fun addEntries(_ v: [AnyStruct])
        ...
        implementation, be mindful of the source you are using and what it does
        */
        access(AddEntry) fun addEntry(_ v: AnyStruct)
    }
    ...
    access(all) resource Raffle: RafflePublic {
        ...
        // UInt64s or Strings
        access(all) let source: @{RaffleSourcePublic, RaffleSourcePrivate}
        ...
        access(contract) fun revealDrawing(id: UInt64): DrawingResult {
            ...
            let ref : auth(FlowtyRaffles.RevealRaffle) &{RaffleSourcePublic, RaffleSourcePrivate} = &self.source
        ...
        access(all) fun addEntries(_ v: [AnyStruct]) {
            self.source.addEntries(v)
        }

        access(all) fun addEntry(_ v: AnyStruct) {
    ...
    */
    access(all) resource interface ManagerPrivate {
        access(all) fun borrowRaffle(id: UInt64): &Raffle?
        access(all) fun commitDrawing(raffleID: UInt64): UInt64
    }
    ...
    access(all) resource Manager: ManagerPublic, ManagerPrivate {
        ...
        */
        access(all) fun createRaffle(source: @{RaffleSourcePublic, RaffleSourcePrivate}, details: Details, revealers: [Address]?): UInt64 {
        ...
        */
        ...
        access(contract) fun _revealDrawing(raffleID: UInt64, receiptID: UInt64, drawer: &Manager) {
            ...
            emit RaffleReceiptRevealed(address: self.owner?.address, raffleID: raffleID, receiptID: receiptID, commitBlock: receiptCommitBlock, revealHeight: getCurrentBlock().height, sourceType: raffle.source.getType(), index: drawingResult.index, value: v, valueType: drawingResult.value.getType())
        }
