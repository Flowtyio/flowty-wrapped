...
access(all) contract FlowtyWrapped: NonFungibleToken, ViewResolver {
    ...
    access(all) struct interface WrappedEdition {
        ...
        access(all) view fun resolveView(_ t: Type, _ nft: &NFT): AnyStruct?
    ...
    ///
    access(all) resource interface FlowtyWrappedCollectionPublic {
        access(all) fun deposit(token: @{NonFungibleToken.NFT})
        access(all) fun getIDs(): [UInt64]
        access(all) fun borrowNFT(id: UInt64): &{NonFungibleToken.NFT}
        access(all) fun borrowFlowtyWrapped(id: UInt64): &FlowtyWrapped.NFT? {
            post {
    ...
    access(all) resource Collection: NonFungibleToken.Collection {
        ...
        access(all) var ownedNFTs: @{UInt64: FlowtyWrapped.NFT}
        ...
        access(all) view fun getSupportedNFTTypes(): {Type: Bool} {
            ...
            supportedTypes[Type<@FlowtyWrapped.NFT>()] = true
        ...
        access(all) view fun isSupportedNFTType(type: Type): Bool {
           if type == Type<@FlowtyWrapped.NFT>() {
            return true
           } else {
            return false
           }
        ...
        /// @return A reference to the wanted NFT resource
        ///        
        access(all) view fun borrowFlowtyWrapped(id: UInt64): &FlowtyWrapped.NFT? {
        ...
        access(all) view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver} {
            let nft = (&self.ownedNFTs[id] as &FlowtyWrapped.NFT?)!
            return nft
        }
    ...
    access(all) resource Admin: AdminPublic {
        ...
        ///
        access(all) fun mintNFT(editionName: String, address: Address, data: {String: AnyStruct}): @FlowtyWrapped.NFT {
        ...
        access(all) fun getEdition(editionName: String): AnyStruct{
            let edition = FlowtyWrapped.getEditionRef(editionName)
            return edition
        }

        access(all) fun registerEdition(_ edition: {WrappedEdition}) {
            pre {
        ...
        access(all) fun setCollectionExternalUrl(_ s: String) {
            ...
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(
                    ...
                    createEmptyCollectionFunction: (fun (): @{NonFungibleToken.Collection} {
                        return <-FlowtyWrapped.createEmptyCollection(nftType: Type<@FlowtyWrapped.NFT>())
                    })
                )
            case Type<MetadataViews.NFTCollectionDisplay>():
    ...
    access(account) fun getRaffleManager(): &FlowtyRaffles.Manager {
        return self.account.capabilities.storage.issue<&FlowtyRaffles.Manager>(FlowtyRaffles.ManagerStoragePath).borrow()!
    }

    access(contract) fun borrowAdmin(): &Admin {
        return self.account.capabilities.storage.issue<&Admin>(self.AdminStoragePath).borrow()!
    }
    ...
    init() {
        ...
        let collectionCap = self.account.capabilities.storage.issue<&FlowtyWrapped.Collection>(self.CollectionStoragePath)
        ...
        let managerCap = self.account.capabilities.storage.issue<&FlowtyRaffles.Manager>(FlowtyRaffles.ManagerStoragePath)
        self.account.capabilities.publish(managerCap, at: FlowtyRaffles.ManagerPublicPath)
